...
export class QueryCache {
    ...
    public query(query: string): PIXI.DisplayObject | undefined {
        ...
        const result = this.findChildRecurrently(this._scene, (child) => child.name === query)
        // this._map.
        // return result;
    }

    public findChildRecurrently(target: PIXI.Container, callback: (child: PIXI.DisplayObject) => boolean): PIXI.DisplayObject {
        const child = target.children.find(callback);
        if (child) {
            return child;
        }

        const containers = target.children
            .filter(value => (value as any).children.length);
