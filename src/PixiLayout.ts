...
class PositionXSetter {
    ...
    protected getValue(width: number): number {
        return (<any>this._value).indexOf ? parseFloat(this._value as string) / 100 * width : this._value as number;
    }
    ...
    public fromRightInwardEdgeOf(source?: ObjectQuery) {
        this._source = source;
        this._xFunc = (source, target) => source.right - target.width - this.getValue(source.width)
        return this.query;
    }

    public fromRightOutwardEdgeOf(source?: ObjectQuery): ILayout {
        this._source = source;
        this._xFunc = (source, target) => source.right + this.getValue(source.width)
        return this.query;
    }
    ...
    public fromLeftOutwardEdgeOf(source?: ObjectQuery): ILayout {
        this._source = source;
        this._xFunc = (source, target) => source.left - target.width - this.getValue(source.width)
        return this.query;
    }
    ...
    public inCenterOf(source?: ObjectQuery): ILayout {
        this._source = source;
        this._xFunc = (source, target) => source.left + source.width / 2 - target.width / 2;
        return this.query;
    }
}
...
class PositionYSetter {
    ...
    protected getValue(height: number): number {
        return (<any>this._value).indexOf ? parseFloat(this._value as string) / 100 * height : this._value as number;
    }
    ...
    public fromBottomInwardEdgeOf(source?: ObjectQuery): ILayout {
        this._source = source;
        this._yFunc = (source, target) => source.bottom - target.height - this.getValue(source.height)
        return this.query;
    }

    public fromBottomOutwardEdgeOf(source?: ObjectQuery): ILayout {
        this._source = source;
        this._yFunc = (source, target) => source.bottom + this.getValue(source.height)
        return this.query;
    }
    ...
    public fromTopOutwardEdgeOf(source?: ObjectQuery) {
        this._source = source;
        this._yFunc = (source, target) => source.top - target.height - this.getValue(source.height)
        return this.query;
    }
    ...
    public inCenterOf(source?: ObjectQuery): ILayout {
        this._source = source;
        this._yFunc = (source, target) => source.top + source.height / 2 - target.height / 2;
        return this.query;
    }
}
...
class WidthSetter {
    ...
    public set = (value: number | string): ILayout => {
        this._value = value;
        this._widthFunc = this._widthFunc || ((source, target) => this.getValue(source.width) / target.width);
        return this.query;
    }
...
class HeightSetter {
    ...
    public set = (value: number | string): ILayout => {
        this._value = value;
        this._heightFunc = this._heightFunc || ((source, target) => this.getValue(source.height) / target.height);
        return this.query;
    }
    ...
    public max = (value: number | string): ILayout => {
        ...
        this._scaleFunc = (source, target) => {
            const calcMax = (<any>value).indexOf ? parseFloat(value as string) / 100 * source.height : value as number;
...
class SizeSetter {
    ...
    public cover(source: ObjectQuery, horizontalAlign: number | keyof typeof Align, verticalAlign?: number | keyof typeof Align): ILayout {
        this._source = source;
        ...
        const alignX = typeof horizontalAlign === "string" ? AlignsMap[horizontalAlign] : horizontalAlign;
        const alignY = verticalAlign !== undefined
            ? (typeof verticalAlign === "string" ? AlignsMap[verticalAlign] : verticalAlign)
            : alignX;
        ...
        this._allFunc = (source, target) => {
            ...
            val.y = source.y + source.height * alignY - (target.height * scale) * alignY;
            return val;
        }
...
type LayoutFunc<returnType = number> = (source: PIXI.Rectangle, target: PIXI.Rectangle, scale?: { x: number, y: number }) => returnType;
...
export class PixiLayout {
    ...
    public calculate(target: DisplayObject, from: ObjectQuery, callbacks: LayoutCallbacks) {
        ...
        if (callbacks["pos"]) {
            const pos = this._callCallback(callbacks["pos"], globalSrcBounds, targetBounds);
            target.position.set((pos.x - targetBounds.x) / targetScale, (pos.y - targetBounds.y) / targetScale);
        }


        if (callbacks["x"]) {
            const x = this._callCallback(callbacks["x"], globalSrcBounds, targetBounds);
            target.x += (x - targetBounds.x) / targetScale / this.globalScale;
        }

        if (callbacks["y"]) {
            const y = this._callCallback(callbacks["y"], globalSrcBounds, targetBounds);
            target.y += (y - targetBounds.y) / targetScale / this.globalScale;
        }


        if (callbacks["width"]) {
            const x = this._callCallback(callbacks["width"], globalSrcBounds, targetBounds);
            target.scale.x *= x;
        ...
        if (callbacks["height"]) {
            const y = this._callCallback(callbacks["height"], globalSrcBounds, targetBounds);
            target.scale.y *= y;
            targetBounds.height *= x;
            if (!callbacks["width"]) {
        ...
        if (callbacks["scale"]) {
            const all = this._callCallback(callbacks["scale"], globalSrcBounds, targetBounds);
            target.scale.set(target.scale.x * all.x, target.scale.y * all.y);
        }

        if (callbacks["size"]) {
            const size = this._callCallback(callbacks["size"], globalSrcBounds, targetBounds);
            (<any>target).width = size.width / targetScale;
            (<any>target).height = size.height / targetScale;
        }

        if (callbacks["all"]) {
            const all = this._callCallback(callbacks["all"], globalSrcBounds, targetBounds);
            target.position.set(target.position.x + (all.x - targetBounds.x) / targetScale / this.globalScale,
                target.position.y + (all.y - targetBounds.y) / targetScale / this.globalScale);
            target.scale.set(target.scale.x * all.scaleX, target.scale.y * all.scaleY);
        }
    }

    private _callCallback<T>(callback: LayoutFuncObj<T>, globalSrcBounds: PIXI.Rectangle, targetBounds: PIXI.Rectangle): T {
    ...
    public createLayout(target: DisplayObject, _query: ILayout) {
        ...
        const setters = [query.sizeSetter, query.xSetter, query.ySetter, query.widthSetter, query.heightSetter];
        const funcs = ["_xFunc", "_yFunc", "_widthFunc", "_heightFunc", "_scaleFunc", "sizeFunc", "_posFunc", "_allFunc"];
        const callbacksKeys = ["x", "y", "width", "height", "scale", "size", "pos", "all"]
...
export function setupPixiLayout(renderer: PIXI.WebGLRenderer | PIXI.CanvasRenderer) {
